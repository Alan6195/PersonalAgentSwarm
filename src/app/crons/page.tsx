...
function CronJobCard({
  ...
  return (
    ...
    >
      <div
        className="p-5 cursor-pointer hover:bg-cream-200 transition-colors"
        onClick={onToggle}
      >
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-medium text-cream-900">{job.name}</h3>
                {!job.enabled && (
                  <span className="badge bg-cream-300 text-cream-500 border-cream-400">
              ...
              <p className="text-[11px] font-mono text-cream-500 mt-1">
      ...
      {isExpanded && (
        <div className="border-t border-cream-300/50 px-5 pb-4">
          <div className="flex items-center gap-2 py-3">
            <Activity className="w-3 h-3 text-cream-500" />
            <p className="text-[10px] font-mono uppercase tracking-widest text-cream-500">
              Recent Runs
            </p>
          </div>
          ...
          {runs && runs.length > 0 ? (
            <div className="space-y-1">
              {runs.map((run) => (
                <div
                  key={run.id}
                  className="flex flex-wrap items-center gap-2 sm:gap-4 py-2 px-3 rounded-lg hover:bg-cream-200/40 transition-colors"
                >
                  <StatusBadge status={run.status} />
                  <span className="text-xs text-cream-500 font-mono flex-1 min-w-[140px]">
                    {new Date(run.started_at).toLocaleString()}
                  </span>
                  <div className="flex items-center gap-3 sm:gap-4 text-xs text-cream-500 font-mono">
                    {run.duration_ms && (
            ...
            <p className="text-xs text-cream-500 py-4 text-center font-mono">
              No runs yet
            </p>
          ) : (
            <div className="space-y-2 py-2">
              {[...Array(3)].map((_, i) => (
                <div
                  key={i}
                  className="h-8 bg-cream-300/20 rounded animate-pulse"
                />
              ))}
            </div>
          )}
    ...
    <div className="space-y-6">
      ...
      {jobs && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="card p-4 text-center">
            <p className="stat-value text-xl text-accent-green">
          ...
          <div className="card p-4 text-center">
            <p className="stat-value text-xl text-accent-red">
        ...
        {!jobs && (
          <div className="space-y-3">
            {[...Array(5)].map((_, i) => (
              <div
                key={i}
                className="card h-24 animate-pulse bg-white/40"
              />
            ))}
          </div>
        )}
